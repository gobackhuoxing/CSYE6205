...
void BookstoreBase::showBooks(){
	...
	sort(booklist.begin(),booklist.end(),[](BookBase* a, BookBase* b){return a->getPrice() < b->getPrice();});
	for(auto book: booklist){
		cout<< *book << endl;
	}

	auto sortbyname = [](BookBase* a, BookBase* b){
...
void BookstoreBase::clean(){
	EmployeeFactory efactory;
	BookFactory bfactory;
	efactory.freeObj(employeelist);
	bfactory.freeObj(booklist);
}
